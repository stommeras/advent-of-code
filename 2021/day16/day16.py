...
filename = sys.argv[1] if len(sys.argv) > 1 else "day16-input.txt"
...
def parse(data):
    ...
    match tid:
        case 0: return sum(spv)
        case 1:
            p = 1
            for x in spv:
                p *= x
            return p
        case 2: return min(spv)
        case 3: return max(spv)
        case 5: return int(spv[0] > spv[1])
        case 6: return int(spv[0] < spv[1])
        case 7: return int(spv[0] == spv[1])
